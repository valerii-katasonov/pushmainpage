...
export function isAudioUrl(u){ return AUDIO_EXT.includes(fileExt(u)); }
...
// ── КЛЮЧ КАЛЕНДАРНОГО ПЛАНУ ─────────────────────────────────────
//
// Той самий курс буває в розкладі двічі — «Математика» і «Matematyka», —
// і план у них один: curriculum_aliases каже, під якою назвою його
// шукати. Ключ плану тому не дорівнює ключу предмета.
//
// ЧОМУ КАРТА ПСЕВДОНІМІВ ПРИХОДИТЬ АРГУМЕНТОМ. Читають план із двох
// дуже різних місць. У кабінеті вчителя карта лежить у кеші
// curriculum.js. А в кабінеті батьків того кеша немає й бути не може:
// картка завантаження плану там не відкривається ніколи, тож і читати
// псевдоніми нікому. Зате там усе одно тягнуть увесь вузол планів
// одним запитом — і карту дістають тим самим запитом поруч.
//
// Функція чиста саме через це: одна логіка на обидва випадки, і не
// доводиться вдруге писати те саме там, де кеша немає. Дві копії цієї
// підстановки — рівно те, чим уже закінчилося з subjKey вище.
//
// Крок один, ланцюжки не розгортаємо: А→Б→В неможливий, бо звʼязати
// предмет із тим, у кого план сам чужий, інтерфейс не дає.
export function planKeyWith(aliases, subj){
  if(!subj) return subjKey(subj);
  const t = aliases && aliases[subjKey(subj)];
  return subjKey((t && String(t).trim()) ? String(t).trim() : subj);
}
...
// НАВІЩО. Оцінки лежать як grades/{клас}/{місяць}/{предмет}/{дата}/{учень} —
// учень останнім сегментом. Прочитати «лише свою дитину» за такої форми
// неможливо: доводиться тягнути весь клас і фільтрувати вже в браузері.
// На екрані видно одну дитину, але в пам'яті вкладки лежить увесь клас, і
// будь-хто з консоллю бачить оцінки однокласників.
//
// РІШЕННЯ. Поруч ведемо дзеркало, де учень стоїть ДРУГИМ сегментом:
//   student_grades/{клас}/{учень}/{місяць}/{предмет}/{дата} = {v, t}
// Батько читає лише свою гілку, і заборона стає властивістю бази, а не
// ввічливістю інтерфейсу.
//
// ЧОМУ ДЗЕРКАЛО, А НЕ ПЕРЕВЕРТАННЯ ОСНОВНОГО ВУЗЛА. Журнал учителя —
// сітка «клас × предмет × дата». За основної форми це вузьке читання
// одного дня; якби учень став другим сегментом, довелося б тягнути весь
// клас за рік. Ми цього тижня саме прибирали такі читання.
//
// ЧОМУ ОДИН update ВІД КОРЕНЯ. Багатошляховий запис у Firebase атомарний:
// або лягають обидва шляхи, або жоден. Розійтися основі й дзеркалу нема як.
export function gradeWritePaths(cls, ym, subj, date, sid, value, type){
  const out = {};
...
export function gradesFromMirror(mirror, name){
  const gr = {}, gt = {};
  for(const subj in (mirror || {})){
    for(const date in mirror[subj]){
      const cell = mirror[subj][date] || {};
      if(!gr[subj]) gr[subj] = {};
      if(!gt[subj]) gt[subj] = {};
      gr[subj][date] = { [name]: cell.v };
      gt[subj][date] = { [name]: cell.t || '' };
    }
  }
  return { gr, gt };
}
...
// pre_approved_roles тримає масив. Проста заміна всього вузла рядком
// «class_teacher» стирала б адміністратора разом із посадою вчителя.
// Тому замінюємо лише викладацьку роль, а інші лишаємо як були.
//
// Правило users/$uid звіряє роль із дітьми 0..3 цього вузла, тож масив
// довший за чотири елементи не спрацює — обрізаємо.
export function withTeachingRole(existing, primary){
  const rest = normalizeRoles(existing).filter(r => r !== 'teacher' && r !== 'class_teacher');
...
export function renderHwItem(subject,data,books){
  ...
  if(typeof data==='object'){
    ...
    const add=u=>{
      ...
      }else if(isAudioUrl(su)){
        const nm=decodeURIComponent(String(su).split('/').pop().split(/[?#]/)[0])||('аудіо.'+ext);
        // Нативний плеєр працює на телефоні й комп'ютері, не запускає
        // запис сам і не вантажить його повністю до натискання Play.
        att+=`<div class="hw-audio"><span>🎧 ${escHtml(nm)}</span>`
           + `<audio controls preload="metadata" src="${escHtml(su)}">`
           + `<a href="${escHtml(su)}" target="_blank" rel="noopener noreferrer">Відкрити аудіо</a>`
           + `</audio></div>`;
      }else{
...
window.submitFirstLogin=async function(ev){
  ...
  try{
    ...
    setBusy('btn-fl-submit',false,'Надіслати лист для входу');
    if(r.ok){
      // Яким листом пішло — у консоль, не на екран. Батькам це знати
      // ні до чого, а нам без цього довелося б лізти в логи Netlify
      // щоразу, коли лист виявиться англійським.
      if(d && d.via === 'firebase'){
        const WHY = {
          'mailer-off':  'у Netlify немає BREVO_API_KEY / MAIL_FROM — або сайт не перезібрано після їх додавання',
          'link-failed': 'Firebase не віддав посилання — перевірте права службового ключа',
          'send-failed': 'Brevo не прийняв лист'
        };
        // «Не підтверджено відправника» була просто найчастішою здогадкою,
        // і саме тому виявилася поганою підказкою: причин у Brevo кілька,
        // а лагодяться вони по-різному. Номер відповіді розрізняє їх точно.
        const BREVO = {
          400: 'адресу у MAIL_FROM не підтверджено в Brevo (Senders → Add a sender)',
          401: 'ключ не підходить — потрібен саме API key v3 (SMTP & API → API keys), не SMTP-пароль',
          402: 'закінчилися безкоштовні листи на сьогодні',
          403: 'акаунт Brevo ще не активовано — нові акаунти вони перевіряють вручну',
            0: 'до Brevo не достукалися'
        };
        console.warn('[Push School] Пішов запасний лист від Firebase.\n'
          + 'Причина: ' + (WHY[d.why] || d.why || 'невідома')
          + (d.mailStatus ? ` (${d.mailStatus})` : '')
          + (BREVO[d.mailStatus] ? '\nСхоже, що: ' + BREVO[d.mailStatus] : '') + '\n'
          + 'Точний текст відмови — Netlify → Functions → first-login → логи.\n'
          + 'Докладніше: НАЛАШТУВАТИ-ЛИСТ-ВХОДУ.md');
      }
    ...
    // Мовчать обидва поштарі. Текст сервер уже склав під причину —
    // «зачекайте пів години» при лічильнику спроб і «зверніться до
    // адміністрації», коли справа в налаштуваннях. Показуємо його як є,
    // а технічну частину кладемо в консоль: причина відмови Brevo тут
    // рівно та сама, що й у гілці із запасним листом, і губити її на
    // найгіршому зі сценаріїв було б безглуздо.
    if(d && (d.code === 'no-mail' || d.code === 'too-many')){
      setMsg('fl-error', d.error, 'login-err');
      console.warn('[Push School] Лист не пішов ЖОДНИМ шляхом.\n'
        + 'Свій лист: ' + (d.why || 'невідомо')
        + (d.mailStatus ? ` (${d.mailStatus})` : '') + '\n'
        + 'Запасний: ' + (d.code === 'too-many'
            ? 'Firebase притримує листи на цю адресу через часті спроби'
            : 'Firebase теж відмовив') + '\n'
        + 'Точні тексти — Netlify → Functions → first-login → логи.');
      return false;
    }
...
function stopAllListeners(){
  const calls = [
    ...
    // Оцінки родини живуть на підписці, а не на разовому читанні —
    // інакше при вході блок лишався порожнім. Знімати обовʼязково:
    // після виходу база сипле permission_denied на кожну з них.
    () => window.stopGradesListeners && window.stopGradesListeners(),
  ];
