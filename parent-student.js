...
window.cancelSelfAttendance=async function(role='parent'){
  ...
  // ОБІД САМ НЕ ПОВЕРНЕТЬСЯ. У кожного прийому їжі своя межа, після якої
  // кухня вже порахувала продукти (сталі *_CUTOFF_HOUR у kitchen.js —
  // годин тут не дублюємо, бо вони змінюються). Якщо родина передумала
  // пізніше, на екрані обід знову зʼявиться, а порції на кухні не буде —
  // і дитина прийде до школи голодною, бо всі вважали, що питання закрите.
...
window.submitAttendance=async function(role='parent'){
  const date=document.getElementById('global-date').value;
  const cls=getActiveClass(), profile=currentUserData;
  const prefix=role==='student'?'s':'p';
